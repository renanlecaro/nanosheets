export function NanoSheets(node, {
    data = {},
    onChange = () => null,
    cellWidth = 200,
    cellHeight = 40,
        ...
        for (let dx = 0; dx < width; dx++) {
            for (let dy = 0; dy < height; dy++) {
                ...
                Object.assign(div.style, {
                    ...
                    top: cellHeight * y + "px",
                    width: cellWidth + "px",
                    height: cellHeight + "px", ...style({
                        x: x,
                        y: y,
                        value: data[cell] || defaultValue({x: x, y: y}),
                        selected: betweenIncluded(x, x1, x2) && betweenIncluded(y, y1, y2),
                        cursor: editing === cell
                    })
                });
                div.textContent = data[cell] || defaultValue({x: x, y: y});
    ...
    function listen(node, ...args) {
        ...
        listeners.push({node: node, args: args})
    }
    ...
    function changeData(changes) {
        ...
        for (const cell in changes) {
            ...
            if ((data[cell] || "") !== (changes[cell] || "")) {
                if (changes[cell] !== defaultValue({x: x, y: y})) {
    ...
    function startEditing(x, y) {
        ...
        if (!editActive || editing !== cell) {
            editing = cell;
            input.value = data[cell] || defaultValue({x: x, y: y});
            editActive = true
        }
        redraw()
    }
    ...
    listen(input, "paste", e => {
        ...
        rows.forEach((row, y) => row.forEach((value, x) => change[dx + x + "_" + (dy + y)] = value));
        changeData(change);
    ...
    function copySelectedToClipboardEvent(e) {
        ...
        for (let y = Math.min(y1, y2); y <= Math.max(y1, y2); y++) {
            ...
            for (let x = Math.min(x1, x2); x <= Math.max(x1, x2); x++) {
                line.push(data[x + "_" + y] || defaultValue({x: x, y: y}))
            }
            asArr.push(line)
        }
    ...
    return {destroy: destroy, scrollTo: scrollTo, redraw: redraw, data: data}
}
